...
<?php
 //echo $form->create('Hierachy',array('action'=>'view_pdf/'.@$per_parent.'/'.@$from.'/'.@$to));
 echo $form->create('Hierachy',array('action'=>'view_pdf/'.@$per_parent));
 ?>
<dl>
 ...
 if(empty($parent_commission_info['MemberCommission']['default_period_start']))
 {
  echo  '-';
 }
 else
 {
  $from = date("Y-m-d",strtotime($parent_commission_info['MemberCommission']['default_period_start']));
  echo $form->text('Hierachy.default_period_start',array('value'=>$from));
 }
 
 echo '&nbsp;-&nbsp;';
 
 if(empty($parent_commission_info['MemberCommission']['default_period_until']))
 {
  echo '-';
 }
 else
 {
  $to = date("Y-m-d",strtotime($parent_commission_info['MemberCommission']['default_period_until'])); 
  echo $form->text('Hierachy.default_period_until',array('value'=>$to)); 
 }
 ?>
 ...
 <dd style="float: left; margin-left: 183px;">
 <?php echo $form->text('Hierachy.miscellaneous'); ?>
 ...
 <?php echo $form->text('Hierachy.remark'); ?>
 <div class="hintz">* Deduction of any kind of activities / cash that still owe the company.</div>
 </dd>
...
<div class="control">
 ...
 <?php echo $html->link('View Tree',array('controller'=>'hierachies','action'=>'tree/'.$per_parent)); ?> 
